...
<h3>apperars in: ${characterData.episode.length} episodes</h3>
</div>
`
...
const makeDomFromData = (data, rootElement) => {
    ...
    if(info.prev){
        buttonsElements.insertAdjacentHTML("beforeend", buttonComponent("prev", `<span class="material-symbols-outlined">
        arrow_back_ios
        </span>`))
    ...
    if(info.next){
        buttonsElements.insertAdjacentHTML("beforeend", buttonComponent("next", `<span class="material-symbols-outlined">
        arrow_forward_ios
        </span>`))
